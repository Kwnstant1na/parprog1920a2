...
// global available messages count
int global_avmsg = 0;
...
void send_msg(Content msg) {
    ...
    // send message
    circus_buffer.array[circus_buffer.in] = msg;
    circus_buffer.in++;
    if(circus_buffer.in == MESSAGES)
    	circus_buffer.in = 0;
    circus_buffer.count++;
...
Content recv_msg() {
    ...
    // receive message
    Content c = circus_buffer.array[circus_buffer.out];
    circus_buffer.out++;
    if(circus_buffer.out == MESSAGES)
    	circus_buffer.out = 0;
    circus_buffer.count--;
